...
## USER SPACE
BLT_USER_PATH = os.getenv("BLT_USER_PATH")
if not BLT_USER_PATH:
    print "$BLT_USER_PATH not set. Exiting ..."
    sys.exit(127)
elif not os.path.isdir(BLT_USER_PATH):
    print "Creating directory %s" % BLT_USER_PATH
    os.makedirs(BLT_USER_PATH)
...
CENTER_ID = 'ORG:53' # Change this if your country is not Sweden.
...
LOGPORT = 8089
if not os.path.isdir(BLT_USER_PATH + "/rave/etc"): os.makedirs(BLT_USER_PATH + "/rave/etc")
LOGFILE     = os.path.join(BLT_USER_PATH + "/rave/etc", "rave_pgf.log") # User space
...
LOGGER_TYPE="logfile" # Can be stdout or logfile but logfile might result in unordered log entries since there will be more than one process writing to same file
