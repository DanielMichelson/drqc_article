...
IGNORE_DETECTORS = ["hac-increment","hac-filter"]  # To avoid unnecessary repetition
...
class compositing(object):
  ...
  def quality_control_objects(self, objects):
    ...
    for k in objects.keys():
      obj = objects[k]
      try:
        detectors = obj.getAttribute("how/algorithm_ids").split(",")
        detectors.insert(-1, "distance")
        detectors.insert(-1, "height")
      except AttributeError:
        detectors = self.detectors
      for d in detectors:
        p = rave_pgf_quality_registry.get_plugin(d)
        if p != None and d not in IGNORE_DETECTORS:
